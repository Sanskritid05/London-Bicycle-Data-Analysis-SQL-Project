...
-- Basic Cleaning and Adding Relevant Cleaned Columns
SELECT
    rental_id,
    bike_id,
...
-- What is the average ride duration, and how is it distributed?
SELECT 
    CASE 
...
-- What are the most popular routes (start-to-end station)?
SELECT
    start_station_name,
    end_station_name,
...
-- How does average ride duration vary across different hours of the day?
SELECT
...
-- What proportion of rides are round trips?
SELECT
    ROUND(
        100 * SUM(
            CASE 
...
-- What is the weekday vs weekend usage split for top stations?
SELECT
    start_station_name,
    SUM(CASE WHEN EXTRACT(DAYOFWEEK FROM start_date) BETWEEN 2 AND 6 THEN 1 ELSE 0 END) AS weekday_rides,
    SUM(CASE WHEN EXTRACT(DAYOFWEEK FROM start_date) IN (1,7) THEN 1 ELSE 0 END) AS weekend_rides
FROM `bigquery-public-data.london_bicycles.cycle_hire`
WHERE
    duration BETWEEN 1 AND 86400
    AND start_station_name IS NOT NULL
GROUP BY start_station_name
HAVING COUNT(*) > 10000
ORDER BY weekday_rides DESC
LIMIT 10;


-- Which routes have unusually long average durations?
WITH route_stats AS (
    SELECT
        start_station_name,
        end_station_name,
